...
function numberReverse() {
  ...
  let number3 = number2.reverse();
  let number4 = number3.join("");
  return number4 * 1;
}

console.log("Number reversed ==>", numberReverse());
...
function stringToOrder() {
  string = string.split("").sort().join("");
  return string;
}
console.log("stringToOrder ==>", stringToOrder());
...
console.log(findGreatest1([3, 4, 5], 4));
console.log(findGreatest1([10, 20, 30], 12));
console.log(findGreatest1([0, 10, 3], 4));
...
console.log(
  `You will need ${calcSupply(
    35,
    2
  )} bars of chocolate (2 a day) to last you till the age of 80.`
);
console.log(
  `You will need ${calcSupply(
    40,
    3
  )} bars of chocolate (3 a day) to last you till the age of 80.`
);
...
function howManyVowels(numx) {
  let vowelsAdded = numx.match(/[aeiou]/gi).length;
  return vowelsAdded;
}
...
console.log("Type of argument is a  => ", detectType(78));
